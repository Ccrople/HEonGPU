...
int main(int argc, char* argv[])
{
    ...
    try
    {
        if (a.model == "regular" || a.model == "slim")
        {
            ...
            const int L = ctos + stoc + taylor + 8 + 2;
            ...
            heongpu::Ciphertext<heongpu::Scheme::CKKS> cipher_boot =
                regular ? operators.regular_bootstrapping(C1, galois_key,
                                                          relin_key)
                        : operators.slim_bootstrapping(C1, galois_key,
                                                       relin_key);
        ...
        else if (a.model == "v2")
        {
            ...
            std::vector<Data64> p_vals = heongpu::generate_proper_primes(
                Data64(2) * Data64(N), 61, 5);
            context->set_coeff_modulus_values(q_vals, p_vals);
            context->generate();

            heongpu::HEKeyGenerator<heongpu::Scheme::CKKS> keygen(context);
            heongpu::Secretkey<heongpu::Scheme::CKKS> secret_key(context, 192);
            ...
            heongpu::Secretkey<heongpu::Scheme::CKKS> sparse_key(context, 32);
